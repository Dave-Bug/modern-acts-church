            ...
            filteredMonthlyRows.length === 0 ? (
              ...
              return (
                <div key={row.category} className="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden">
                  {/* Card Header */}
                  <div className={`p-4 flex items-center justify-between gap-3 ${row.bg} border-b ${row.border}`}>
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-2.5">
                        <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${row.bg.replace('50', '100')} ${row.color}`}>
                          {row.icon}
                        </div>
                        <span className="font-bold text-base text-slate-900">{row.category}</span>
                      </div>
                    </div>
                    <div className="text-right flex-shrink-0">
                      <div className={`text-lg font-black ${row.color}`}>₱{formatCurrency(row.total)}</div>
                    </div>
                  </div>
                  ...
                  <div className="p-4">
                    <div className="grid grid-cols-5 gap-2">
                      {row.weeklyData.map((week, widx) => {
                        ...
                        return (
                          <div key={widx} className={`py-2.5 px-1 rounded-lg flex flex-col items-center justify-center min-h-[56px] ${
                            hasValue
                              ? "bg-slate-50 text-slate-800 border border-slate-200"
                              : "bg-white text-slate-300 border border-slate-100"
                          }`}>
                            <span className="text-[11px] font-bold text-slate-400 mb-1">W{widx + 1}</span>
                            <span className="font-mono font-bold text-xs">{hasValue ? formatCurrency(week.amount) : "—"}</span>
                          </div>
                        );
                      })}
                    </div>
            ...
            filteredAnnualRows.length === 0 ? (
              ...
              <div key={row.category} className="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden">
                <div className={`p-4 flex items-center justify-between gap-3 ${row.bg} border-b ${row.border}`}>
                  ...
                  <div className="text-right flex-shrink-0">
                    <div className="text-lg font-black text-slate-900">₱{formatCurrency(row.total)}</div>
                  </div>
                </div>
                <div className="p-4">
            ...
            <div className="overflow-x-auto w-full">
              <table className="w-full text-left border-collapse min-w-[600px]">
                ...
                <tbody className="divide-y divide-slate-100 text-xs sm:text-sm">
                  {filteredMonthlyRows.length === 0 ? (
                    <tr><td colSpan={weeks.length + 3} className="py-12 text-center text-slate-400 text-base">No income records found for this period.</td></tr>
                  ) : filteredMonthlyRows.map((row) => (
                    <tr key={row.category} className="hover:bg-slate-50/60 transition-colors">
                      ...
                      {row.weeklyData.map((week, widx) => (
                        <td key={widx} className="py-3 px-1 sm:px-2 text-center">
                          <span className={`font-mono font-bold text-xs sm:text-sm ${week.amount > 0 ? row.color : 'text-slate-300'}`}>
                  ...
                  <tr className="bg-slate-100/50 font-black border-t border-slate-200">
                    <td className="py-3 px-3 sm:px-4 text-slate-900 text-[10px] uppercase">Grand Total</td>
                    {weeks.map((week, idx) => (
                    ...
                    <td className="py-3 px-2 sm:px-3 text-right font-mono text-emerald-500">₱{formatCurrency(monthlyGrandTotal)}</td>
                  </tr>
                </tbody>
              </table>
            </div>
            ...
            <div className="overflow-x-auto w-full">
              <table className="w-full text-left border-collapse min-w-[700px]">
                ...
                <tbody className="divide-y divide-slate-100 text-xs sm:text-sm font-semibold">
                  {filteredAnnualRows.length === 0 ? (
                    <tr><td colSpan={14} className="py-12 text-center text-slate-400 text-base">No income records found for this year.</td></tr>
                  ) : filteredAnnualRows.map((row) => (
                    <tr key={row.category} className="hover:bg-slate-50/60 transition-colors">
                  ...
                  <tr className="bg-slate-100/50 font-black border-t border-slate-200">
                    <td className="py-3 px-4 text-slate-900 text-[10px] uppercase">Grand Total</td>
                    ...
                    <td className="py-3 px-3 text-right font-mono text-emerald-500">₱{formatCurrency(annualGrandTotal)}</td>
                  </tr>
                </tbody>
              </table>
            </div>
