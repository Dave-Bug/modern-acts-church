...
import { FaHome, FaSearch, FaSpinner, FaUser, FaHandHoldingHeart } from "react-icons/fa";
...
export default function FinancePledge() {
  ...
  async function fetchPledgeData() {
    try {
      ...
      console.error("Error fetching pledge data:", err.message);
    } finally {
      setLoading(false);
    }
  }
  ...
  const displayedPledges = filteredPledges.slice(0, 15);
  const totalAmount = displayedPledges.reduce((sum, item) => sum + parseFloat(item.amount || 0), 0);
      ...
      <div className="max-w-6xl mx-auto px-4 py-8 md:py-10 pt-16">
        ...
        <div className="text-center mb-6">
          <h1 className="text-2xl md:text-4xl font-black">
            Pledge <span className="text-blue-600">Records</span>
          </h1>
        </div>
        ...
        <div className="grid grid-cols-2 gap-3 mb-6">
          <div className="bg-white/80 backdrop-blur border border-slate-200 rounded-lg p-3 md:p-4">
            ...
            <p className="text-lg md:text-xl font-bold text-slate-900">{displayedPledges.length}</p>
          </div>
        ...
        <div className="bg-white/80 backdrop-blur border border-slate-200 rounded-xl overflow-hidden">
          {loading ? (
            ...
            <div className="py-8 text-center">
              <p className="text-sm text-slate-400">No matching pledge records found.</p>
            </div>
          ) : (
          ...
          {!loading && filteredPledges.length > 15 && (
            <div className="bg-slate-50 px-4 py-2 border-t border-slate-100 text-center">
              <p className="text-[10px] text-slate-400">Showing 15 of {filteredPledges.length} entries</p>
            </div>
          )}
        </div>
      </div>
