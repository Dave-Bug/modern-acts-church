...
export default function Ministries() {
    ...
    if (sessionStr) {
      try {
        ...
        if (parsedUser.ministry) {
          const list = parsedUser.ministry.split(",").map((m) => m.trim().toLowerCase());
  ...
  // 🛡️ ROLE & ASSIGNMENT CHECKER FUNCTION
  const hasAccess = (matchKey) => {
    if (!currentUser) return false;
    if (currentUser.status === "Admin") return true; // Admin override bypass
  ...
  // --- 🔐 ON LOGIN SUCCESS WITHIN OVERLAY ---
    ...
    <section className="min-h-screen flex flex-col bg-[#f5f7fb] text-slate-900 relative overflow-hidden">
      ...
      {currentUser && (
        <div className="fixed top-4 right-4 z-40 flex items-center gap-3 bg-white/90 backdrop-blur border border-slate-200 shadow-lg px-4 py-2 rounded-xl">
          ...
          <span className={`text-[9px] font-black uppercase tracking-wider px-2 py-0.5 rounded-md ml-1 ${
            currentUser.status === "Admin" ? "bg-red-50 text-red-600 border border-red-100" :
            currentUser.status === "Editor" ? "bg-amber-50 text-amber-600 border border-amber-100" :
          ...
          {ministries.map((m, i) => {
            ...
            return (
              <Link
                ...
                className={`
                  group bg-white/80 backdrop-blur border border-slate-200 rounded-xl p-4 md:p-5 transition-all duration-300 block relative
                  ${allowed 
              ...
              >
                ...
                <div className={`w-10 h-10 rounded-lg flex items-center justify-center mb-3 ${allowed ? `${theme.bg} ${theme.icon}` : "bg-slate-200 text-slate-400"}`}>
                  <Icon />
                </div>
                ...
                <div className={`flex items-center gap-2 text-xs font-semibold transition ${allowed ? "text-slate-400 group-hover:text-blue-600" : "text-slate-300"}`}>
                  {allowed ? "ENTER PORTAL" : "LOCKED"}
                  <FaArrowRight size={10} className={allowed ? "group-hover:translate-x-1 transition" : "hidden"} />
                </div>
              </Link>
            );
          })}
